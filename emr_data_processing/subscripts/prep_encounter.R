...
# 4. Smoking Status 
#   a. create new var set to factor with current, former, never, unknown as levels
...
prep_encounter <- function(temp){
  ...
  temp %<>%
    ...
    mutate(Weight_lbs = ifelse(Weight_lbs > 600 | Weight_lbs < 80, NA, Weight_lbs))
  
  # max(temp$Weight_lbs, na.rm = TRUE)
  ...
  medicaid <- c("Managed Medicaid", "Colorado Medicaid", "Out of State Medicaid")
...
if(sum(expected_smoke == actual_smoke) == 10){
  ...
  # Modify vitals --------------------------------------------------------------
  # clean_asterisks() will replace any values that start with "*" with NA
  # temp %<>% 
  #   clean_asterisks()
  
  # # Set vitals variables that need modification to numeric
  # cols_to_mod <- c("HeartRate", "Respiratoryrate", "Systolic_blood_pressure",
  #                  "Diastolic_blood_pressure", "Temperature", "Respiratoryrate",
  #                  "BMI")
  
  # # Set *Restricted values to NA at the specified columns and convert them to 
  # # numeric to allow the calculation of summary statistics (means, medians, etc)
  # temp %<>% mutate_at(cols_to_mod, as.numeric)
  ...
  # Although 57 clinics were randomized, there are actually only 56 as one clinic
