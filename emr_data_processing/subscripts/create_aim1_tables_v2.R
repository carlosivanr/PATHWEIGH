...
# May 6, 2024
# Create tables for the aim 1 manuscript version 2

# Description: This function will generate gt_summary tables in word format
# displaying a number of health metrics, types of visits, and calls quarto
# documents to generate stand alone word documents for displaying ggplots and
# lmer model output

# Dependencies: This script was designed to work on the mod_data data frame as
# input so that only those that were entered in the model could be displayed.
# However, requests were made to display analyzed (mod_data[["ee"]]), and all
# eligible enrolled patients in some tables and hence the make_tables() function
# works as a function and a script because tables relying on visits_post_id use
# the data frame from the workspace.
...
make_tables <- function(data){
  ...
  c("Control", "Intervention") %>%
    ...
    gt::gtsave(filename = here("tables",
                               str_c("table_1_",
                                     date_min, "_",
                                     Sys.Date(),
                                     gt_ext)))
  ...
  invisible(
    file.copy(
      ...
      to = here("tables", 
                str_c("clinic_engagement_", date_min, "_", Sys.Date(), ".docx")),
      TRUE))
  ...
  # Set the final file output name
  file_name <- "lmer_model_output_and_figure.docx"
  
  # Set the qmd file input path
  file_in_path  <-  str_c(emr_dir, "report_layouts/lmer_model_output_and_figure.qmd")
  ...
  invisible(
    file.copy(
      ...
      # to = here("tables", file_name),
      to = here("tables", 
                str_c("lmer_model_output_and_figure_", date_min, "_", Sys.Date(), ".docx")),
      TRUE))
