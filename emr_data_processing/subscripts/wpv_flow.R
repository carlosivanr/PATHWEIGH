...
wpv_flow <- function(temp){
  ...
  # # How many encounters have evidence of the PW_flow, but not the OBHPI
  # temp %>%
  #   filter(WPV_PW_flow == 1, WPV_OBHPI == 0) %>%
  #   nrow()
  # 
  # # How many encounters have evidence of the OBHPI, but not the PW_flow
  # temp %>%
  #   filter(WPV_PW_flow ==0, WPV_OBHPI == 1) %>%
  #   nrow()
  # 
  # # How many encounters have evidence of PW Flow and OBHPI
  # # Should be 0
  # temp %>%
  #   filter(WPV_PW_flow == 1, WPV_OBHPI == 1) %>%
  #   nrow()
  ...
  # How many rows do not have mutual information in PW_flow and WMQ? ----------
  # temp %>%
  #   filter(WPV_PW_flow ==1, WPV_WMQ == 1) %>%
  #   nrow()
  # 
  # temp %>%
  #   filter(WPV_PW_flow ==0, WPV_WMQ == 1) %>%
  #   nrow()
  # 
  # temp %>%
  #   filter(WPV_PW_flow ==1, WPV_WMQ == 0) %>%
  #   nrow()
