...
set_index_date_ee_ene <- function(temp){
  ...
  if ("IndexDate" %in% colnames(temp)){
    ...
    # Capture the Arb_PersonIds of the Eligible and Enrolled
    ee_pts <- ee_ene %>%
      filter(Eligible == 1, Enrolled == 1) %>%
      select(Arb_PersonId) %>%
      distinct()
    
    # Capture the index date as the first encounter that meets criteria
    # ProviderNpi is not missing, Weight is not missing, but it no longer
    # needs to be a WPV
    index_dates <- temp %>%
    ...
    left_join(temp, index_dates,
      ...
      mutate(IndexVisit = if_else(EncounterDate == IndexDate & IndexVisitEligible == 1, 1, 0))
    
    # Number of patients that have more thane one legitimate IndexVisit
    temp %>% 
      filter(IndexVisit == 1) %>% 
