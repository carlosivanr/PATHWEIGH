...
labs_procedures <- function(temp){
  ...
  if (sum(map_lgl(dfs, exists)) != length(dfs)){
    stop("Not all required data frames are loaded into the workspace. Check that procedure, labs, flowsheets, and referrals are in properly named.")
  }
  ...
  temp %>% 
    ...
    select(Arb_PersonId, Arb_EncounterId, EncounterDate, Censored, IndexDate, IndexVisit, IndexVisitEligible, Intervention)
  ...
  prep_screener <- function(screener_data){
    # pull encounter ids for records that have inconsistent values across the same instrument, encounter and patient
    inc_enc_ids <- 
      screener_data %>%
      filter(Value != "") %>%
    ...
    # Remove encounter ids with multiple yet different values for the same encounter
    screener_data %<>%
  ...
  # Procedures (O2, CPAP, BIPAP) V1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  # Create a data frame where procedures are joined with distinct_pts_x_ind by
  # patient ID, then filtered for those with O2, CPAP, or BIPAP, and within 
  # the time window of 30 days after the visit and 30 days before the visit. 
  # Then procedures are grouped and counted in a summarise statement. At this 
  # point, each count can be replaced by 1 to indicate they have O2CPAPBIPAP
  # regardless of the amount of records found within the time window
  ...
  temp <-
    left_join(temp, O2CPAPBIPAP_data, by= c("Arb_PersonId", "IndexDate")) %>%
    # group_by(Arb_PersonId) %>%
    # fill(O2CPAPBIPAP, .direction = "updown") %>%
    # ungroup() %>%
    mutate(O2CPAPBIPAP = ifelse(is.na(.$O2CPAPBIPAP), 0, O2CPAPBIPAP)) #%>%
    # pull(O2CPAPBIPAP) %>%
    # table(.)
  ...
  # Bariatric Surgery V2 -------------------------------------------------------
  # May 2024, decision to look at any procedure within each period for each 
  # patient.
  
  # Year 0 March 17, 2020 - March 16, 2021, Baseline
  # Year 1 March 17, 2021 - March 16, 2022, Group 1 crossed over to intervention
  # Year 2 March 17, 2022 - March 16, 2023, Group 2 crossed over to intervention
  # Year 3 March 17, 2022 - March 16, 2024, Group 3 crossed over to intervention
  ...
  #   mutate(across(Ref_BariatricSurgery:Ref_WellnessClinic, ~ifelse(is.na(.x), 0, .x)))
  ...
  # toc()
  return(temp)
  #  } #Closing curly brace to the test for labs & procedures commented to 
  #allow processing of the last visit
}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
