...
labs_procedures <- function(temp, int){
  ...
  if (sum(grepl("O2CPAPBIPAP", names(temp))) == 0){
    ...
    # then filtered for those with O2, CPAP, or BIPAP, and within the time window
    # of 30 days after the visit and 30 days before the visit. Then procedures
    # are grouped and counted in a summarise statement. At this point, every
    # patient will have at most two rows, and each count can be replaced by 1 to
    # indicate they have O2CPAPBIPAP, because they have been filtered.
