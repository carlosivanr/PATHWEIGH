...
# lmer model with index visits and without baseline
```{r}
# Load pp model data
...
pp_data %<>%
  ...
  group_by(Arb_PersonId, Intervention) %>%
  # fill(Year_at_ind, .direction = c("downup", "updown")) %>%
  fill(Age_cat, .direction = "downup") %>%
  ungroup()

pp_data %<>%
  mutate(N_days_post_180 = ifelse(IndexVisit == 1, 0, N_days_post_180))
```
...
tab %>%
  ...
  gt::rows_add(
    ...
    .before = "0") %>%
  gt::tab_header(
    title = "Model w index visits and w/ out baseline",
    # subtitle = "subtitle place holder"
  )
```
...
```{r}
# Create figure of observed vs predicted values
predicted <- predict(lmer_mod_bin, pp_data, re.form=NA, type="response")

obs_pred <- bind_cols(pp_data, data.frame(predicted)) %>%
  mutate(observed = Weight_dv)
```

```{r, dpi = 600, fig.width=6.5}
# 3-line plot of the per-protocol model
obs_pred %>%
  mutate(group = ifelse(slope1 == 1, "Con", "Int")) %>%
  select(Arb_PersonId, N_months_post_id, observed, predicted, group) %>%
