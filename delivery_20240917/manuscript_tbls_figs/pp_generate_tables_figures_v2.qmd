...
overlay_data %<>%
  ...
  pivot_longer(cols = fit:upr, names_to = "Type", values_to = "Avg_Weight_kgs") %>%
...
pan_a +
  ...
  geom_line(data = overlay_data %>% filter(Type == "lwr"), color = "red", linetype = "dotted") +
  geom_line(data = overlay_data %>% filter(Type == "upr"), color = "red", linetype = "dotted")
```
```{r, fig.height = 4.2, fig.width=6.5, fig.dpi = 600}
...
```{r}
# mean(pp_mod_data$slope1)
# 
# mean(pp_mod_data$slope2)
# 
# mean(pp_mod_data$slope3)
# 
# mean(pp_mod_data$slope4)
# 
# pp_mod_data %>% 
#   summarise(across(slope1:slope4, ~ mean(.x, na.rm = TRUE)))
# 
# 
# pp_mod_data %>% 
#   select(slope1:slope4) %>%
...
### All panels display predicted values from 4-slope model
...
```{r}
#| eval: false
# correlation = nlme::corSymm(form = ~ 1|Arb_PersonId),
# correlation = nlme::corCompSymm(form = ~ 1|Arb_PersonId), # Matches defaul lmerTest
broom.mixed::tidy(
  nlme::lme(
    A1C ~ Visit * Intervention.factor, 
    random = ~ 1| Arb_PersonId,
    correlation = nlme::corSymm(form = ~ 1|Arb_PersonId),
    data = (ind_vs_lv %>% drop_na(A1C)))
  # conf.int = TRUE,
  )  %>%
  as_tibble() %>%
  mutate_if(is.numeric, ~round(., 3)) %>%
...
```{r}
#| eval: false
# Display intervention only
visits_post_id %>%
  filter(Arb_EncounterId %in% pp_data$Arb_EncounterId,
         IndexVisit == 1,
         Intervention.factor == "Intervention") %>%
  select(all_of(ordered_comorbidity_names)) %>%
  tbl_summary()
```
...
```{r}
#| eval: false
# Why does this person not have a last visit in control???
# Need to use the Arb_PersonId, and not the EncounterId to filter
visits_post_id %>%
  filter(Arb_PersonId %in% pp_data$Arb_PersonId) %>%
  filter(IndexVisit == 1 | LastVisit == 1) %>%
  # group_by(Intervention.factor, Arb_PersonId) %>%
  arrange(Intervention.factor, Arb_PersonId, EncounterDate) %>%
  select(Arb_PersonId, IndexVisit, LastVisit, N_days_post_id, Intervention.factor) %>%
